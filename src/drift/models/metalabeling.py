...
class MetaLabelling(Composite):
    ...
    def preprocess_y_secondary(self, y: pd.Series, results_primary: List[pd.DataFrame]) -> pd.Series:
        assert len(results_primary) == 1 and is_prediction(results_primary[0]), ValueError("Primary pipeline needs to be concluded with a prediction.")
        
        meta_y = pd.concat([discretized_predictions, y], axis=1).apply(
