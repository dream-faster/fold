...
def train(
    ...
    if train_method == TrainMethod.parallel_with_search and len(splits) > 1:
        process_function = __get_process_function(backend)
...
def __get_process_function(backend: Backend) -> Callable:
    if backend == Backend.ray:
        return _train_transformations_ray
    else:
        return _train_transformations_sequential
