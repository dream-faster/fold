...
transformations_over_time = train(pipeline_arima_1, X, y, splitter)

## Evaluating the model
pred = backtest(transformations_over_time, X, y, splitter)

sc = score(y[pred.index], pred.squeeze(), "Pipeline_ARIMA_1").save()
...
## Training the model
splitter = ExpandingWindowSplitter(initial_train_window=0.15, step=0.1)
transformations_over_time = train(pipeline_arima_2, X, y, splitter)

## Evaluating the model
pred = backtest(transformations_over_time, X, y, splitter)

sc = score(y[pred.index], pred.squeeze(), "Pipeline_ARIMA_2").save()
sc.print_summary(extended=False)
...
# %%
pipeline_ensemble = WrapStatsForecast(ARIMA, {"order": (2, 1, 1)}, use_exogenous=False)
